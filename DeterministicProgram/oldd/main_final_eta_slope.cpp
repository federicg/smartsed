        ...
        switch ( M_frictionModel )
        {
            ...
            case 1:
                ...
                for ( const auto & Id : idStaggeredInternalVectHorizontal )
                {
                    ...
                    Real alfa = 1.;
                    const Real theta = std::atan( slope_x[ Id ] );
                    const Real den = std::pow( H_interface[ Id ] * std::cos( theta ), M_expo );
                    if ( den > M_H_min )
                    {
...
//                    if ( den > M_H_min )
//                    {
//                        M_gamma_dt_DSV_x = std::max( M_gamma_dt_DSV_x_[ Id ] / std::max( std::pow( H_interface[ Id ] * std::abs( u[ Id ] ), M_expo_r_x_vect[ Id ] ), M_toll ), M_gamma_dt_DSV_x );
////                        std::cout << M_gamma_dt_DSV_x_[ Id ] / std::max( std::pow( H_interface[ Id ] * std::abs( u[ Id ] ), M_expo_r_x_vect[ Id ] ), 1.e-10 ) << std::endl;
//                        alfa = 1. / ( 1. + M_gamma_dt_DSV_x * std::abs( u[ Id ] ) / den );
//                    }
                    
                    const Real theta = std::atan( slope_x[ Id ] );
                    const Real den = std::pow( H_interface[ Id ] * std::cos( theta ), M_expo + M_expo_r_x_vect[ Id ] );
                    if ( den > M_H_min )
                    {
                        alfa = 1. / ( 1. + M_gamma_dt_DSV_x_[ Id ] * std::pow( std::abs( u[ Id ] ), 1. - M_expo_r_x_vect[ Id ] ) / den );
                        alfa = std::min( alfa, 1. / ( 1. + M_gamma_dt_DSV_x * std::abs( u[ Id ] ) / std::pow( H_interface[ Id ] * std::cos( theta ), M_expo ) ) );
                    }
...
//                    }
                ...
                for ( const auto & Id : idStaggeredBoundaryVectEast )
                {
                    //M_gamma_dt_DSV_x = std::max( M_gamma_dt_DSV_x_[ Id ] / std::pow( std::max( H_interface[ Id ], H_min ) * std::abs( u[ Id ] ), M_expo_r_x_vect[ Id ] ), M_gamma_dt_DSV_x );
                     ...
                     const std::vector<UInt>& idStaggeredBoundaryVectSouth )
    {
        ...
        switch ( M_frictionModel )
        {
            ...
            case 1: 
                ...
                {
                    Real alfa = 1.;
                    const Real theta = std::atan( slope_y[ Id ] );
                    const Real den = std::pow( H_interface[ Id ] * std::cos( theta ), M_expo );
                    if ( den > M_H_min )
                    {
...
//                    if ( den > M_H_min )
//                    {
//                        M_gamma_dt_DSV_y = std::max( M_gamma_dt_DSV_y_[ Id ] / std::max( std::pow( H_interface[ Id ] * std::abs( v[ Id ] ), M_expo_r_y_vect[ Id ] ), M_toll ), M_gamma_dt_DSV_y );
//                        alfa = 1. / ( 1 + M_gamma_dt_DSV_y * std::abs( v[ Id ] ) / den );
//                    }
                    
                    const Real theta = std::atan( slope_y[ Id ] );
                    const Real den = std::pow( H_interface[ Id ] * std::cos( theta ), M_expo + M_expo_r_y_vect[ Id ] );
                    if ( den > M_H_min )
                    {
                        alfa = 1. / ( 1 + M_gamma_dt_DSV_y_[ Id ] * std::pow( std::abs( v[ Id ] ), 1. - M_expo_r_y_vect[ Id ] ) / den );
                        alfa = std::min( alfa, 1. / ( 1. + M_gamma_dt_DSV_y * std::abs( v[ Id ] ) / std::pow( H_interface[ Id ] * std::cos( theta ), M_expo ) ) );
                    }
...

{
    ...
    for ( const auto & Id : idBasinVect )
    {
        const auto IDreIndex = idBasinVectReIndex[ Id ];
        coefficients.push_back( Eigen::Triplet<Real>( IDreIndex,  IDreIndex,  1. ) );
        rhs( IDreIndex ) = ( eta( Id ) + (precipitation[ Id ] * dt_DSV) ) * alp;
//        if (precipitation[ Id ] != 0) std::cout << precipitation[ Id ] * dt_DSV /* * ((precipitation[ Id ] * dt_DSV) >= 1.e-10)*/ << std::endl;
    }
    ...
    for ( const auto & Id : idStaggeredInternalVectHorizontal )
    {
        ...
        if ( H_interface > H_min )
        {
            const auto coeff_matrix = c3 * coeff_m * std::cos( std::atan( slope_x[ Id ] ) );
            coefficients.push_back( Eigen::Triplet<Real>( IDleftReIndex, IDrightReIndex, - coeff_matrix ) );
            coefficients.push_back( Eigen::Triplet<Real>( IDrightReIndex, IDleftReIndex, - coeff_matrix ) );
            
            
            coefficients.push_back( Eigen::Triplet<Real>( IDleftReIndex,  IDleftReIndex,   coeff_matrix ) );
            coefficients.push_back( Eigen::Triplet<Real>( IDrightReIndex, IDrightReIndex,  coeff_matrix ) );
    ...
    for ( const auto & Id : idStaggeredBoundaryVectWest )
    {
        ...
        if ( H_interface > H_min )
        {
            const auto coeff_matrix = c3 * coeff_m * std::cos( std::atan( slope_x[ Id ] ) );
            rhs( IDrightReIndex ) += isNonReflectingBC *
                                   ( - c1 * ( - coeff_m  * u_star[ Id ] ) - ( orography[ IDrightright ] - orography[ IDright ] ) * coeff_matrix ) * alp;
        }
    ...
    for ( const auto & Id : idStaggeredBoundaryVectEast )
    {
        ...
        if ( H_interface > H_min )
        {
            const auto coeff_matrix = c3 * coeff_m * std::cos( std::atan( slope_x[ Id ] ) );
            rhs( IDleftReIndex ) += isNonReflectingBC *
                                  ( - c1 * ( + coeff_m  * u_star[ Id ] ) - ( orography[ IDleftleft ] - orography[ IDleft ] ) * coeff_matrix ) * alp;
        }
    ...
    for ( const auto & Id : idStaggeredInternalVectVertical )
    {
        ...
        if ( H_interface > H_min )
        {
            const auto coeff_matrix = c3 * coeff_m * std::cos( std::atan( slope_y[ Id ] ) );
            coefficients.push_back( Eigen::Triplet<Real>( IDleftReIndex, IDrightReIndex, - coeff_matrix ) );
            coefficients.push_back( Eigen::Triplet<Real>( IDrightReIndex, IDleftReIndex, - coeff_matrix ) );
    ...
    for ( const auto & Id : idStaggeredBoundaryVectNorth )
    {
        ...
        if ( H_interface > H_min )
        {
            const auto coeff_matrix = c3 * coeff_m * std::cos( std::atan( slope_y[ Id ] ) );
            rhs( IDrightReIndex ) += isNonReflectingBC *
                                   ( - c1 * ( - coeff_m  * v_star[ Id ] ) - ( orography[ IDrightright ] - orography[ IDright ] ) * coeff_matrix ) * alp;
        }
    ...
    for ( const auto & Id : idStaggeredBoundaryVectSouth )
    {
        ...
        if ( H_interface > H_min )
        {
            const auto coeff_matrix = c3 * coeff_m * std::cos( std::atan( slope_y[ Id ] ) );
            rhs( IDleftReIndex )  += isNonReflectingBC *
                                   ( - c1 * ( + coeff_m  * v_star[ Id ] ) - ( orography[ IDleftleft ] - orography[ IDleft ] ) * coeff_matrix ) * alp;
        }
...
//            std::cout << H_interface << std::endl;
            v[ Id ] = isNonReflectingBC * alfa_y[ Id ] * ( v_star[ Id ] - c2 * std::cos( std::atan( slope_y[ Id ] ) ) * ( orography[ Id + N_cols ] - orography[ Id ] ) );
    ...
    for ( const auto & Id : idStaggeredBoundaryVectSouth )
    {
        ...
        if ( H_interface > H_min )
        {
            v[ Id ] = isNonReflectingBC * alfa_y[ Id ] * ( v_star[ Id ] - c2 * std::cos( std::atan( slope_y[ Id ] ) ) * ( orography[ Id - N_cols ] - orography[ Id - 2*N_cols ] ) );
        }
        else
        {
            v[ Id ] = 0.;
    ...
    for ( const auto & Id : idStaggeredInternalVectHorizontal )
    {
        ...
        if ( H_interface > H_min )
        {
            u[ Id ] = alfa_x[ Id ] * ( u_star[ Id ] - c2 * std::cos( std::atan( slope_x[ Id ] ) ) * ( eta( IDeast ) - eta( IDwest ) ) );
        }
        else
        {
            u[ Id ] = 0.;
    ...
    for ( const auto & Id : idStaggeredBoundaryVectWest )
    {
        ...
        if ( H_interface > H_min )
        {
            u[ Id ] = isNonReflectingBC * alfa_x[ Id ] * ( u_star[ Id ] - c2 * std::cos( std::atan( slope_x[ Id ] ) ) * ( orography[ Id - i + 1 ] - orography[ Id - i ] ) );
        }
        else
        {
            u[ Id ] = 0.;
    ...
    for ( const auto & Id : idStaggeredBoundaryVectEast )
    {
        ...
        if ( H_interface > H_min )
        {
            u[ Id ] = isNonReflectingBC * alfa_x[ Id ] * ( u_star[ Id ] - c2 * std::cos( std::atan( slope_x[ Id ] ) ) * ( orography[ Id - i - 1 ] - orography[ Id - i - 2 ] ) );
        }
        else
        {
            u[ Id ] = 0.;
    ...
    {
        {
            ...
            if ( HMat.cellsize <= pixel_size )
            {
                ...
                "H=resample(H,dem,method='bilinear');" +
...
//        exit(1);
    ...
    eta_basin.resize( numberCellsInBasin );
    rhs      .resize( numberCellsInBasin );
    
    
    
    // maybe if eta_basin is inizialized the restart
    if ( restart_H )
    {
        UInt h = 0;
        for ( UInt i = 0; i < N_rows; i++ )
        {
            for ( UInt j = 0; j < N_cols; j++ )
            {
                ...
                if ( basin_mask_Vec[ k ] == 1 )
                {
                    eta_basin( h ) = ( H( k ) + orography[ k ] ) * scale_coeff;
                    h++;
                }
            }
        }
    ...
    else
    {
        ...
        for ( const UInt & Id : idBasinVect )
        {
            ...
            eta_basin( IDreIndex ) = orography[ Id ] * scale_coeff;
        }
    }
    ...
    for ( Int n = 1; n <= nstep; n++ )
    {
        ...
        else
        {
            ...
            eta_basin = cg.solveWithGuess( rhs, eta_basin );
...
//        saveVector(eta_basin, output_dir+"H_"+std::to_string(n)+".txt");
        
        for ( const UInt & Id : idBasinVect )
        {
            const UInt IDreIndex = idBasinVectReIndex[ Id ];
            eta( Id ) = eta_basin( IDreIndex ) / scale_coeff;
        }
        ...
        for ( const UInt & k : idBasinVect )
        {
            H( k ) = eta( k ) - orography[ k ];
        }
        
        std::cout << "min H: " << H.minCoeff() << " max H: " << H.maxCoeff() << std::endl;
