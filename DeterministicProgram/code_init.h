...
void resize_rasters( 
    ...
    std::vector<double>& basin_mask_Vec, 
    std::vector<double>& orography, 
    std::vector<double>& h_G, 
    std::vector<double>& h_sd, 
    std::vector<double>& h_sn, 
    std::vector<double>& S_coeff,
    std::vector<double>& W_Gav, 
    std::vector<double>& W_Gav_cum, 
    std::vector<double>& hydraulic_conductivity, 
    std::vector<double>& Z_Gav, 
    std::vector<double>& d_90, 
    ...
    std::vector<double>& h_interface_x, 
    std::vector<double>& h_interface_y, 
    std::vector<double>& slope_x,
    std::vector<double>& slope_y, 
    std::vector<double>& slope_cell, 
    std::vector<double>& soilMoistureRetention, 
    std::vector<double>& roughness_vect, 
    ...
    double& xllcorner, 
    double& yllcorner, 
    double& xllcorner_staggered_u, 
    double& yllcorner_staggered_u,
    double& xllcorner_staggered_v, 
    double& yllcorner_staggered_v, 
    double& NODATA_value,
   
    const int currentSimNumber,
    const int rank,
    const int scaling_factor,
    const std::string friction_model,
    std::string& output_dir, const std::string file_dir,
    const bool save_temporal_sequence,
    const double max_Days,
    std::string& temperature_file,
    std::string& ET_model,
    std::string& infiltrationModel,
    double& phi_rad,
    double& dt_min,
    const double dt_temp,
    double& slope_x_max, double& slope_y_max,
    const int number_gauges,
    std::vector<std::vector<unsigned int>>& kk_gauges )

{

    output_dir =
        "../Outputs/" + std::to_string(currentSimNumber) + "/";
    ...
    const bool restart_H =
        dataFile("files/initial_conditions/restart_H", false);
    ...
    infiltrationModel =
        dataFile("files/infiltration/infiltration_model", "None");
    ...
    // Resize the input Geometry
    {
      ...
      if (basin_mask.cellsize <= pixel_size) {
        ...
        const std::string bashCommand =
            ...
            "writeRaster( basin, file=paste0('" + output_dir +
            "basin_mask.asc'), overwrite=TRUE )\"";
    ...
    if (restart_H) {
      {
        ...
        if (HMat.cellsize <= pixel_size) {
          const std::string bashCommand =
              ...
              "writeRaster( H, file=paste0('" + output_dir +
              "H_0.asc'), overwrite=TRUE )\"";
          std::system(bashCommand.c_str());
      ...
      saveSolution(output_dir + "H_0", " ", N_rows, N_cols, xllcorner,
                   yllcorner, pixel_size, NODATA_value, H);
    }

    if (restart_vel) {
      {
        const std::string restart_vel_u_file = dataFile(
            "files/initial_conditions/vel_file_u", "restart_vel_u.txt");
        Raster vel_u_Mat(file_dir + restart_vel_u_file);

        if (vel_u_Mat.cellsize <= pixel_size) {
          const std::string bashCommand =
              std::string("Rscript -e ") + "\"library(raster);" +
              "dem=raster('" + output_dir + "DEM.asc" + "');" + "u=raster('" +
              file_dir + restart_vel_u_file + "');" + "dem_u=dem;" +
              ...
              "writeRaster( u, file=paste0('" + output_dir +
              "u_0.asc'), overwrite=TRUE )\"";
          std::system(bashCommand.c_str());
      ...
      {
        const std::string restart_vel_v_file = dataFile(
            "files/initial_conditions/vel_file_v", "restart_vel_v.txt");
        Raster vel_v_Mat(file_dir + restart_vel_v_file);

        if (vel_v_Mat.cellsize <= pixel_size) {
          const std::string bashCommand =
              std::string("Rscript -e ") + "\"library(raster);" +
              "dem=raster('" + output_dir + "DEM.asc" + "');" + "v=raster('" +
              file_dir + restart_vel_v_file + "');" + "dem_v=dem;" +
              ...
              "writeRaster( v, file=paste0('" + output_dir +
              "v_0.asc'), overwrite=TRUE )\"";
          std::system(bashCommand.c_str());
      ...
      saveSolution(output_dir + "u_0", "u", N_rows, N_cols, xllcorner,
                   yllcorner, pixel_size, NODATA_value, u);
      ...
      saveSolution(output_dir + "v_0", "v", N_rows, N_cols, xllcorner,
                   yllcorner, pixel_size, NODATA_value, v);
    }
    ...
    if (restart_snow) {
      {
        ...
        if (snow_Mat.cellsize <= pixel_size) {
          const std::string bashCommand =
              std::string("Rscript -e ") + "\"library(raster);" +
              "dem=raster('" + output_dir + "DEM.asc" + "');" + "hsn=raster('" +
              file_dir + restart_snow_file + "');" +
    ...
    if (restart_sediment) {
      {
        ...
        if (sediment_Mat.cellsize <= pixel_size) {
          const std::string bashCommand =
              std::string("Rscript -e ") + "\"library(raster);" +
              "dem=raster('" + output_dir + "DEM.asc" + "');" + "hsd=raster('" +
              file_dir + restart_sediment_file + "');" +
    ...
    if (restart_gravitational) {
      {
        ...
        if (gravitational_Mat.cellsize <= pixel_size) {
          const std::string bashCommand =
              std::string("Rscript -e ") + "\"library(raster);" +
              "dem=raster('" + output_dir + "DEM.asc" + "');" + "hG=raster('" +
              file_dir + restart_gravitational_file + "');" +
      ...
      saveSolution(output_dir + "hG_0", " ", N_rows, N_cols, xllcorner,
                   yllcorner, pixel_size, NODATA_value, h_G);
    }
...

    {
      ...
      if (restart_soilMoisture) {
        ...
        str1 = output_dir + "clay_sim_" + std::to_string(currentSimNumber) +
               ".asc";
        str2 = output_dir + "sand_sim_" + std::to_string(currentSimNumber) +
               ".asc";
          ...
          for (unsigned int j = 0; j < N_cols; j++) {
            ...
            } else if (sand < .5 && sand >= 0 && clay > .4 && clay <= 1) // D
            {
              ...
              for (auto ii = 0; ii < vv.size(); ii += 4) {
                ...
                if (e2.dot(point - D) >= 0 &&
                    e2.dot(point - D) <= (A(1) - D(1)))
                  d2 = std::abs((point - D).dot(e1));
                if (e1.dot(point - A) >= 0 &&
                    e1.dot(point - A) <= (B(0) - A(0)))
                  d3 = std::abs((point - A).dot(e2));
                if (e2.dot(point - C) >= 0 &&
                    e2.dot(point - C) <= (B(1) - C(1)))
      ...
      saveSolution(output_dir + "d_10", " ", N_rows, N_cols, xllcorner,
                   yllcorner, pixel_size, NODATA_value, d_10);
      saveSolution(output_dir + "d_90", " ", N_rows, N_cols, xllcorner,
                   yllcorner, pixel_size, NODATA_value, d_90);
      saveSolution(output_dir + "k_c", " ", N_rows, N_cols, xllcorner,
                   yllcorner, pixel_size, NODATA_value, hydraulic_conductivity);
    }
    ...
    for (int number = 1; number <= number_gauges; number++) {
      ...
      if (save_temporal_sequence) {
        ...
        if (XX(0) < 0 || XX(1) < 0 || XX(1) >= N_rows || XX(0) >= N_cols) {
          if (rank == 0)
            std::cout << "The gauges in the input file are not good"
                      << std::endl;
          exit(1.);
        }
    ...
    if (static_subbasin_approx) {
      ...
      for (const auto &Id : idBasinVect) {
        if (counter_near_excl[Id] ==
            4) // it is surely an isolated cell! (can be also an excluded cell
               // but no problem..)
        {
          std::get<0>(excluded_ids[Id]) = true;
        }
