...
int main(int argc, char **argv) {
  ...
  if (totSimNumber >= 0 && rank == 0) {
    const std::string bashCommand =
        ...
        std::to_string(command_line.follow(2, "-scale"));
  ...
  for (int currentSimNumber =
    ...
    const std::string temperature_file =
    ...
    if (constant_precipitation || !is_precipitation) {
      ...
      precipitation.constant_precipitation(file_dir + precipitation_file,
    ...
    } else // IDW
    {
      ...
      precipitation.IDW_precipitation(
    ...
    while (!is_last_step) {
      ...
      if (rank == 0) {
        std::cout << "Simulation progress: " << time / t_final * 100 << " %"
                  ...
                  << maxCourant(H, g, c1_DSV_) << std::endl;
