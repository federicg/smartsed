  ...
  if (ff.is_open()) {
    ...
    if (format_temp == "comune") {
      ...
      for (unsigned int i = 0; i < ndata; i++) {
        ...
        if (std::abs((i % rr) * time_spacing - hour_full) >
            .1 * (i % rr) *
                time_spacing) //( std::abs( ((i%rr)+1)*time_spacing - hour_full
                              //) > .1 * ((i%rr)+1)*time_spacing )
        {
...
double bilinearInterpolation(const std::vector<double> &u,
  ...
  double output_hor;
  {
    ...
    auto x_1 = std::floor(x), y_1 = std::floor(y), x_2 = x_1 + 1, y_2 = y_1 + 1;
    ...
    const auto Id_11 = x_1 + y_1 * (ncols + 1), Id_12 = x_1 + y_2 * (ncols + 1),
  ...
  for (unsigned int i = 0; i < N_rows; i++) {
    for (unsigned int j = 0; j < N_cols; j++) {
      ...
      if (i != (N_rows - 1)) {
        ...
        if ((basin_mask_Vec_mpi[IDcell] + basin_mask_Vec_mpi[IDcell_south]) ==
            2) {
          idStaggeredInternalVectVertical_mpi.push_back(
