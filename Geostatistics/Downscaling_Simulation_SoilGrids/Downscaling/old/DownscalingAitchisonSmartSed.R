...
for (i in 1:length(sim1@layers)){
  ...
  writeRaster( aux_clay, file=paste0('Geostatistics/clay_sim_',i,'.tif'), overwrite=TRUE )
  writeRaster( aux_sand, file=paste0('Geostatistics/sand_sim_',i,'.tif'), overwrite=TRUE )
  writeRaster( aux_silt, file=paste0('Geostatistics/silt_sim_',i,'.tif'), overwrite=TRUE )
}
