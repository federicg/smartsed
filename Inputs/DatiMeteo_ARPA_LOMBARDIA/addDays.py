...
origFNameL = {'carenno':'carenno', 'barzio_PDB':'barzioPDB', 'canzo':'canzo', 'cassina':'cassina','cortenova':'cortenova', 'lecco':'lecco', 'rotaImagna':'rotaImagna', 'valmadrera':'valmadrera','sormano':'sormano'}
fNameStep = {'carenno': 0.1667, 'barzio_PDB': 0.1667, 'canzo': 0.1667, 'cassina': 0.1667,'cortenova': 0.1667, 'lecco': 0.1667, 'rotaImagna': 0.1667, 'valmadrera': 1.0,'sormano': 0.1667}
...
for oFNL in origFNameL:
  ...
  nameF = (oFNL+'/rain_'+oFNLv+'_ev.txt')
  outF  = (oFNL+'/rain_'+oFNLv+'_ev0.txt')
  fOut = open(outF,'w')
  hourL = 3600
    ...
    for ii in range(22,iniDay):
      ...
      if (iniDay-ii < 7):
        ...
        if  ii < thr2: 
          ...
          preRain_scal = preRain#*((thr1-ii)*2+1)
      else:
        preRain_scal = preRain
      preRainC = "{:2.1f}".format(preRain_scal)
      ...
      for jj in range(0,int(24*hourL/step)):
        ...
        ## Id Day Hour Value
        fOut.write("{} {} {} {}\n".format(idFlag,dateTot,timeStr,preRainC))
      #break
    while ll :
      lsep = ll.strip().split()
      #print(lsep)
